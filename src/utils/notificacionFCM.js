...
async function enviarNotificacionesProximas() {
    try {
        ...
        for (const prog of programaciones) {
            ...
            // ── Ya fue Tomado u Omitido hoy → no notificar ────────────────
            if (ultimoEstado === 'Tomado' || ultimoEstado === 'No Tomado') {
                continue;
            }

            // ── Pospuesto → renotificar si pasaron ~5 minutos ─────────────
            ...
            if (proximaSlot >= ahora && proximaSlot <= en5min) {
                // Evitar duplicados: ¿ya se mandó notificación en los últimos 6 min?
                const [reciente] = await db.query(`
                    SELECT idToma FROM historial_tomas
                    WHERE id_programacion_fk = ?
                      AND creado_en >= DATE_SUB(NOW(), INTERVAL 6 MINUTE)
                    LIMIT 1
                `, [prog.idProgramacion]);
